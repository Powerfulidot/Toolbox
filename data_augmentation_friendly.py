...
def changeLight(img):                                                      # 亮度
    alpha = random.uniform(0.35, 1)
    blank = np.zeros(img.shape, img.dtype)
    return cv2.addWeighted(img, alpha, blank, 1 - alpha, 0)

def cutout(img, bboxes, length=100, n_holes=1, threshold=0.5):       # 随机裁剪
    '''
...
def rotate_img_bbox(img, bboxes, angle=5, scale=1.):                        # 旋转
    '''
...
def crop_img_bboxes(img, bboxes):                        # 裁剪
    '''
    裁剪后的图片要包含所有的框
    输入:
        img:图像array
...
def shift_pic_bboxes(img, bboxes):                                   # 平移
    '''
    平移后的图片要包含所有的框
    输入:
        img:图像array
...
def filp_pic_bboxes(img, bboxes):                                            # 翻转
    '''
        平移后的图片要包含所有的框
        输入:
            img:图像array
...
# 保存图片结果
def save_img(file_name, save_folder, img):
...
for parent, _, files in os.walk(source_img_path):
    ...
    for file in files:
        ...
        values = parse_xml(xml_path)  # 解析得到box信息，格式为[[x_min,y_min,x_max,y_max,name]]
