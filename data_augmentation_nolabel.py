'''
Description   : 无需xml格式标签的图片数据扩充脚本
Time          : 2024/11/22 16:45:04
Author        : 宽后藤 
'''
...
class DataAugmentForObjectDetection():
    def __init__(self, rotation_rate=0.5, max_rotation_angle=10,
        ...
        self.is_changeLight = False
        self.is_cutout = False
        self.is_rotate_img_bbox = is_rotate_img_bbox
        self.is_crop_img_bboxes = False
        self.is_shift_pic_bboxes = False
    ...
    # ---4.旋转--- #
    def _rotate_img_bbox(self, img, angle=5, scale=1.):
        '''
    ...
    # ---6.平移--- #
    # def _shift_pic_bboxes(self, img):
    #     '''
    #     平移后的图片要包含所有的框
    #     输入:
    #         img:图像array
        ...
        while change_num < 1:  # 默认至少有一种数据增强生效
            ...
            if self.is_rotate_img_bbox:
                if random.random() > self.rotation_rate:  # 旋转
                    ...
                    img = self._rotate_img_bbox(img, angle, scale)
 
            if self.is_shift_pic_bboxes:
                if random.random() < self.shift_rate:  # 平移
                    change_num += 1
                    img = self._shift_pic_bboxes(img)
            ...
            if self.is_cutout:
                if random.random() < self.cutout_rate:  # cutout
                    change_num += 1
                    img = self._cutout(img, length=self.cut_out_length, n_holes=self.cut_out_holes,
                                       threshold=self.cut_out_threshold)
            if self.is_filp_pic_bboxes:
                if random.random() < self.flip_rate:  # 翻转
                    change_num += 1
                    img = self._filp_pic_bboxes(img)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source_img_path', type=str, default='C:/Users/29560/Desktop/lucitic/')
    # parser.add_argument('--source_xml_path', type=str, default='C:/Users/29560/Desktop/defects/train_xml')
    parser.add_argument('--save_img_path', type=str, default='C:/Users/29560/Desktop/4090/')
    # parser.add_argument('--save_xml_path', type=str, default='C:/Users/29560/Desktop/defects/aug_xml')
    ...
    for parent, _, files in os.walk(source_img_path):
        ...
        for file in files:
            ...
            while cnt < need_aug_num:  # 继续增强
                auged_img = dataAug.dataAugment(img)
