'''
Description   : Retinex及其衍生算法
Time          : 2024/10/30 11:34:17
Author        : 宽后藤 
'''
...
for imgs in file_list:
    ...
    # img = cv2.imread('C:/Users/29560/Desktop/babaji/8.jpg')

    # img_ssr = ssr(img, sigma=80)
    # img_msr = msr(img)
    # img_msrcr = msrcr(img)
    img_msrcp = msrcp(img)

    # cv2.namedWindow('img', cv2.WINDOW_NORMAL)
    # cv2.namedWindow('img_ssr', cv2.WINDOW_NORMAL)
    # cv2.namedWindow('img_msr', cv2.WINDOW_NORMAL)
    # cv2.namedWindow('img_msrcr', cv2.WINDOW_NORMAL)
    # cv2.namedWindow('img_msrcp', cv2.WINDOW_NORMAL)
    ...
    cv2.imwrite(os.path.join(save_path, imgs.name), img_msrcp)
    print(imgs.name)
